...
MSIRS_immunization_dynamics <- function(times,y,parms){
  ...
  mabs_catchup1 = parms$monoclonal_catchup_01[times]
  mabs_catchup2 = parms$monoclonal_catchup_23[times]
  mabs_catchup3 = parms$monoclonal_catchup_45[times]
  mabs_catchup4 = parms$monoclonal_catchup_67[times]
  waningN=1/(parms$waningN/length.step) #duration of nirsevimab protection
  waningN2=1/(parms$waningN2/length.step)
  ...
  V_s60=parms$senior_vax_60_74[times]
  ...
  dy[,'N1'] <- c(mabs_catchup1,mabs_catchup2,mabs_catchup3,mabs_catchup4,rep(0,10)) - #from both M0 and S0 compartments
    RRIn*lambda*N1 -
    waningN*N1-
    (mu + um)*N1 +
  ...
  dy[,'S0'] <- omega*M0 -
    c(mabs_catchup1,mabs_catchup2,mabs_catchup3,mabs_catchup4,rep(0,10)) -
    lambda*S0 -
    (mu + um)*S0 +
    Aging.Prop*c(0,S0[1:(N.ages-1)])

  dy[,'I1'] <-  lambda*S0+ lambda*Si+ RRIn*lambda*N1 +RRIn*lambda*N2 +
    parms$sigmaM*lambda*M0 +  RRIn*parms$sigmaM*lambda*Mn1 +RRIn*lambda*Mn2 + RRIv*parms$sigmaM*lambda*Mv1 +RRIv*lambda*Mv2 -
    gamma1*I1-
    (mu + um)*I1 +
