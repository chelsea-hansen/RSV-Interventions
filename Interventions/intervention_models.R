intervention_models <- function(t,y,parms, time.step='week'){
  ...
  dy[,'M'] <- (1-birth_n)*period.birth.rate*sum(States) - 
    lambda*M - 
    V_n*sum(States[1:4,"M"]) - #infants who receive nirsevimab after birth (but still in M compartment)
    (omega+(mu+um))*M +
  ...
  dy[,'N'] <- V_n*sum(States[1:4,"M"]) + V_n*sum(States[1:4,"S0"]) - 
    waningN*N-
    RRIn*lambda*N -
    (mu + um)*N + 
