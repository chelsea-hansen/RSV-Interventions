MSIRS_interventions <- function(t,y,parms){
  ...
  cup_N = parms$cover_n[t] #infants who receive nirsevimab catch-up dose  
  ...
  birth_V = parms$maternal_dose[t]#infants born to vaccinated mothers
  ...
  dy[,'M'] <- (1-(birth_N+birth_V))*period.birth.rate*sum(States) - 
  ...
  dy[,'I1'] <-  lambda*S0+ lambda*Si+ RRIn*lambda*N +
    parms$sigma3*lambda*M +  RRIn*parms$sigma3*lambda*Mn + RRIv*parms$sigma3*lambda*Mv- 
    (gamma1 + mu + um)*I1 + 
