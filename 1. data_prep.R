...
#based on POLYMOD contact matrix 
...
#starting values for model compartments (proportion of each age group in each compartment)
  ...
  seed = sum(pop$population)/100000 #seeding 1 infection per 100K population each week - convert to days 
  
  vital_stats = get_estimates(
    geography ="county",
    product = "components",
    breakdown_labels = TRUE,
    state = "WA",
  ...
  #covert births to weekly number of births 
  births = vital_stats %>% filter(variable=="BIRTHS") %>% select(value)/52.1775
  birth=c(births$value, rep(0,12))
  
  #covert to weekly net migrations per person 
  mig_rate = vital_stats %>% filter(variable=="RNETMIG") %>% select(value)/1000
  um = mig_rate$value*-1/52.1775
 
  #save all fixed parameters with population data 
  ...
  all_dat = list(parmset,yinit, yinit.vector)
saveRDS(all_dat,"Data/fixed_parameters.rds")
